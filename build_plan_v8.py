...
OUT = "/home/user/Feypk/Plan_RU_Econ_Quant_v8.xlsx"

FILL = {
    "econ":  PatternFill("solid", fgColor="DDEBF7"),  # ฟ้า = เศรษฐศาสตร์
    "quant": PatternFill("solid", fgColor="E2EFDA"),  # เขียว = คณิต-สถิติ-โปรแกรม
    "fin":   PatternFill("solid", fgColor="FCE4D6"),  # ส้ม = การเงิน/บัญชี (ECO/FIN/ACC)
    "gen":   PatternFill("solid", fgColor="F2F2F2"),  # เทา = ศึกษาทั่วไป
    "new":   PatternFill("solid", fgColor="FFF2CC"),  # เหลือง = วิชาเพิ่มใหม่ v8
}
...
PLAN = [
 ("ปีที่ 1 / เทอม 1", 21, [
   ...
   ("ACC1103","การบัญชีการเงิน",3,"แกน econ","-","พื้นบัญชี → บุพวิชา FIN2101/ACC2202","econ"),
   ...
   ("COS1101","Introduction to Computer Science",3,"เก็บเพิ่ม • พื้นฐาน CS ไม่มีบุพวิชา","-","ปลดล็อกสาย COS","quant"),
 ]),
 ("ปีที่ 1 / เทอม 2", 21, [
   ...
   ("COS1103","Algorithms and Programming",3,"เก็บเพิ่ม • ต่อจาก COS1101 • ปลดล็อก COS2102 (C++)","COS1101","เลิกต้องลุ้นเทียบ Python","quant"),
 ]),
 ("ปีที่ 2 / เทอม 1", 21, [
   ...
   ("ECO2129","เศรษฐกิจประเทศไทย",3,"แกน econ (เรียนหลัง ECO1122)","ECO1121,1122","-","econ"),
 ...
 ("ปีที่ 3 / เทอม 2", 21, [
   ...
   ("MTH4404","Introduction to Real Analysis 1",3,"เก็บเพิ่ม • สัญญาณ #1 ของ grad school","MTH2104(+MTH2001)","Real Analysis (MIT 18.100)","quant"),
 ...
 ("ปีที่ 4 / เทอม 1", 21, [
   ...
   ("ECO4125","Microeconometrics",3,"เชี่ยวชาญ (เลือก) • เช็กว่ามี causal inference","ECO3123","IV/DiD/RDD?","econ"),
   ("ECO4326","Mathematical Finance",3,"เลือกกลุ่มอื่น–การเงิน","ECO3323","Mathematical Finance","fin"),
   ("MTH3611","Mathematical Stochastic Models",3,"เก็บเพิ่ม • Stochastic (quant finance/derivatives)","MTH2106","หัวใจ quant finance","quant"),
   ("ACC2202","Intermediate Accounting",3,"★ เพิ่ม v8 • บัญชีเชิงลึก (ฐานก่อน ACC4209)","ACC1103 (เช็กภาควิชา)","สาย IB ต้องอ่านงบเป็น","new"),
   ("FIN3211","Financial Management (Corporate Finance)",3,"★ เพิ่ม v8 • การเงินองค์กร — เนื้อ IB","FIN2101 (อาจต้อง FIN3208 — เช็ก)","capital structure/budgeting","new"),
 ...
 ("ปีที่ 4 / เทอม 2", 21, [
   ...
   ("COS2102","Object-Oriented Programming (C++)",3,"เก็บเพิ่ม • C++ (quant dev) — เก็บไว้ตามที่ขอ","COS1103","✅ ปลดล็อกจาก COS1103","quant"),
   ...
   ("ACC4209","Financial Reporting & Analysis",3,"★ เพิ่ม v8 • วิเคราะห์งบการเงิน (core IB/ER)","ACC2202 (เช็กภาควิชา)","financial statement analysis","new"),
   ("RAM1211","Principles of Entrepreneurship",3,"ศึกษาทั่วไป 1.2 (ผู้ประกอบการ)","-","-","gen"),
 ]),
]

wb = Workbook(); ws = wb.active
ws.title = "แผนรายเทอม v8"
ws.sheet_view.showGridLines = False
NC = 7

ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=NC)
c = ws.cell(1,1,"แผนการเรียน ป.ตรี เศรษฐศาสตร์ ม.รามคำแหง — สาย Quant + Finance/IB (v8: 168 นก.)")
c.font = FONT_TITLE; c.fill = TITLE_FILL; c.alignment = CENTER
ws.row_dimensions[1].height = 24

ws.merge_cells(start_row=2, start_column=1, end_row=2, end_column=NC)
note = ("แก้จาก v7: เอนเข้าสาย Finance/IB มากขึ้นแต่คงแกน Quant • ตัด MTH4405 (Real Analysis 2), "
        "MTH3407 (Numerical), FIN3319 (Fintech) • เพิ่ม ACC2202 (Intermediate Accounting), "
        "ACC4209 (Financial Reporting & Analysis), FIN3211 (Corporate Finance) • เก็บ C++ (COS2102) ไว้ • รวม 168 นก. เท่าเดิม")
...
c = ws.cell(3,1,"ฟ้า=เศรษฐศาสตร์ | เขียว=คณิต–สถิติ–โปรแกรม | ส้ม=การเงิน/บัญชี | เทา=ศึกษาทั่วไป | เหลือง=วิชาเพิ่มใหม่ v8 (ACC2202, ACC4209, FIN3211)")
...
summ = [
 ...
 ("เก็บเพิ่ม (STEM+CS+Finance+ACC)","-","48","เอน IB: +ACC/CorpFin, −math reach"),
...
for j,w in enumerate([4,10,42,7,40,26,30], start=1):
